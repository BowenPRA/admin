...
import { bandFor, mealRateFor, periodInfo, PROGRAMS } from './fees.js'
import { pct, fmtDate } from './money.js'
...
export function defaultStudentOptions(student, fees, calendar, ctx) {
  ...
  const upper = bandFor(student.level) === 'upper' || ['hybrid', 'independent'].includes(student.program)
  return {
...
export function buildDocument(inputs, fees, calendar) {
  ...
  let discountId = null
  if (inputs.siblingDiscount && entries.length >= 2 && !['staff', 'trial', 'none'].includes(plan)) {
    if (inputs.siblingStudentId && entries.some((e) => e.student.id === inputs.siblingStudentId)) {
  ...
  // ---------- Tuition section ----------
  const isUpperAny = entries.some((e) => e.opts.upper)
  const showDiscountCol = discountId !== null

  if (!['staff', 'trial', 'none'].includes(plan) && entries.length) {
    ...
    if (plan === 'earlyBird' || plan === 'standard') {
      ...
      heading = L(lang, `TUITION FEE FOR ${(entries[0]?.student.program === 'global' ? 'GLOBAL' : entries[0]?.student.program === 'vocational' ? 'VOCATIONAL' : 'SHORT-TERM')} PROGRAM, YEAR ${sy}`,
